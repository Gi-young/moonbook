			...
			<form method="post" id="frm" action="${path}/usedboard/usedboardUpdateEnd.do?usedboard_No=${no}" enctype="multipart/form-data">
				<ul>
					...
					<li>
						...
						<input type="checkbox" name="usedboard_Category" id="category3" value="청소년문학" <c:set var="i" value="${i=0 }"/> <c:forEach var="b" items="${usedboard.usedboard_Category}"> <c:set var="i" value="${i+1 }"/>  ${usedboard.usedboard_Category[i-1].contains("청소년문학")?"checked":"" }</c:forEach>><label for="category3" class="">청소년문학</label>&nbsp;
						...
						<input type="checkbox" name="usedboard_Category" id="category8" value="세계명작동화" <c:set var="i" value="${i=0 }"/> <c:forEach var="b" items="${usedboard.usedboard_Category}"> <c:set var="i" value="${i+1 }"/>  ${usedboard.usedboard_Category[i-1].contains("세계명작동화")?"checked":"" }</c:forEach>><label for="category8" class="">세계명작동화</label>&nbsp;
						<input type="checkbox" name="usedboard_Category" id="category9" value="한국전래동화" <c:set var="i" value="${i=0 }"/> <c:forEach var="b" items="${usedboard.usedboard_Category}"> <c:set var="i" value="${i+1 }"/>  ${usedboard.usedboard_Category[i-1].contains("한국전래동화")?"checked":"" }</c:forEach>><label for="category9" class="">한국전래동화</label>&nbsp;
						<input type="checkbox" name="usedboard_Category" id="category10" value="국내창작동화" <c:set var="i" value="${i=0 }"/> <c:forEach var="b" items="${usedboard.usedboard_Category}"> <c:set var="i" value="${i+1 }"/>  ${usedboard.usedboard_Category[i-1].contains("국내창작동화")?"checked":"" }</c:forEach>><label for="category10" class="">국내창작동화</label>&nbsp;
						<input type="checkbox" name="usedboard_Category" id="category11" value="다른나라그림책" <c:set var="i" value="${i=0 }"/> <c:forEach var="b" items="${usedboard.usedboard_Category}"> <c:set var="i" value="${i+1 }"/>  ${usedboard.usedboard_Category[i-1].contains("다른나라그림책")?"checked":"" }</c:forEach>><label for="category11" class="">다른나라그림책</label>&nbsp;
						<input type="checkbox" name="usedboard_Category" id="category12" value="어린이동화(초1~2학년)" <c:set var="i" value="${i=0 }"/> <c:forEach var="b" items="${usedboard.usedboard_Category}"> <c:set var="i" value="${i+1 }"/>  ${usedboard.usedboard_Category[i-1].contains("어린이동화(초1~2학년)")?"checked":"" }</c:forEach>><label for="category12" class="">어린이동화(초1~2학년)</label>&nbsp;
						<input type="checkbox" name="usedboard_Category" id="category13" value="어린이동화(초3~4학년)" <c:set var="i" value="${i=0 }"/> <c:forEach var="b" items="${usedboard.usedboard_Category}"> <c:set var="i" value="${i+1 }"/>  ${usedboard.usedboard_Category[i-1].contains("어린이동화(초3~4학년)")?"checked":"" }</c:forEach>><label for="category13" class="">어린이동화(초3~4학년)</label>&nbsp;
						<input type="checkbox" name="usedboard_Category" id="category14" value="어린이동화(초5~6학년)" <c:set var="i" value="${i=0 }"/> <c:forEach var="b" items="${usedboard.usedboard_Category}"> <c:set var="i" value="${i+1 }"/>  ${usedboard.usedboard_Category[i-1].contains("어린이동화(초5~6학년)")?"checked":"" }</c:forEach>><label for="category14" class="">어린이동화(초5~6학년)</label>&nbsp;
