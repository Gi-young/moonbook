						...
						<table class="adminHowTable">
							<tr>
								...
								<td><input type="button" value="조회하기"
									onclick="return orderList3(1,10);" class="adminBtn"></td>
							</tr>
							<tr>
								<th>검색하기</th>
...
function adStockDelete(event){
	let bindNo=event.target.parentElement.parentElement.
...
function orderList3(cPage, numPerpage) {
	...
	if(Number(stockNum1)>=Number(stockNum2)){
		alert("대소 비교를 다시 확인하세요.");
		return false;
	}else{
		if(typeT === "stockBookT"){
			$.ajax({
